...
def make_reservation():
    ...
    if not db.is_customer_has_less_than_two_reservations_this_week(customer_id, date):
...
def export_to_json(from_date, to_date):
    data = {
        date.strftime('%d.%m.%Y'):
                [
                    {
...
def export_to_csv(from_date, to_date):
